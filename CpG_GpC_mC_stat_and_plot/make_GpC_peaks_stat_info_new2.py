...
# -*- encoding: utf-8 -*-
'''
@File    :   make_GpC_peaks_stat_info.py
...
def get_genome_gpc_count(genome_fasta):
    ...
    genome_gpc_count = 0
    gpc_pattern = re.compile(r'GC[ACT]')
    for seq_record in SeqIO.parse(genome_fasta, "fasta"):
        sequence = seq_record.seq.upper()
        rev_comp_seq = sequence.reverse_complement()
        genome_gpc_count += len(gpc_pattern.findall(str(sequence)))
        genome_gpc_count += len(gpc_pattern.findall(str(rev_comp_seq)))
...
def get_gpc_count_from_fasta(tab_fasta_file, gpc_dict):
    ...
    with open(tab_fasta_file, 'r') as tfin:
        for line in tfin:
            ...
            gpc_dict[seq_name] = len(gpc_pattern.findall(str(sequence)))
            gpc_dict[seq_name] += len(gpc_pattern.findall(str(rev_comp_seq)))
    return gpc_dict
...
def main():
    ...
    with open(merged_gpmc_file, 'r') as merge_in, open(output_file, 'w') as fout:
        ...
        for line in merge_in:
            ...
            # get poisson p value
            peak_pv_based_lambda_genome = poisson.pmf(mc_count, lambda_genome)
            peak_pv_based_lambda_l3 = poisson.pmf(mc_count, lambda_l3)
            peak_pv_based_lambda_k5 = poisson.pmf(mc_count, lambda_k5)
            peak_pv_based_lambda_k10 = poisson.pmf(mc_count, lambda_k10)
            peak_pv_based_lambda_k50 = poisson.pmf(mc_count, lambda_k50)
            peak_pv_based_lambda_max = poisson.pmf(mc_count, lambda_max)
            # output
